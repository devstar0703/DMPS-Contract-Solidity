...
contract('Marketplace Contract', async () => {
    ...
    it('nft', async () => {
        let current_block_timestamp = await nftMarketplace.getCurrentTimeStamp() ;

        let utcTime = Number(current_block_timestamp.toString()) * 1000 ;

        console.log(new Date(utcTime).toUTCString());
        console.log(new Date());

        let mint_price = ethers.utils.parseUnits('0.0035', 'ether') ;
        let amount = 1 ;

        await nftMarketplace.mintNFT(amount , { value : mint_price * (amount - 1)  });

        amount = 1 ;

        await nftMarketplace.mintNFT(amount , { value : mint_price * (amount - 1)  });

        let balance = await nftMarketplace.balanceOfMinter() ;
